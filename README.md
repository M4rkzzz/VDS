...
你可以把它理解成一个更可控、更偏专业共享场景的 Oopz / QQ / KOOK / Discord 屏幕分享替代方案：不把重点放在群聊、语音房和社交功能上，而是专注于画面共享、多人观看、OBS 接入、链式转发和自部署可控。

它更适合那些普通聊天软件屏幕分享不太顺手的场景：小团队远程协作、内网演示、游戏或软件测试围观、教学排障、临时直播预览，以及需要多人同时看同一画面的局域网环境。

> **置顶推荐**
>
> 如果你觉得 Oopz、QQ、KOOK、Discord 的屏幕分享在画质、链路、人数、局域网、OBS 接入或可控性上不够顺手，VDS 就是为这类需求准备的。房主只需要选择画面并开始共享，观看者拿到房间号或在公开房间里进入；当观看人数变多时，系统会尽量让观看端接力转发，减少房主单点上行压力。
>
> 简单说：它不是聊天软件，也不是会议套件，而是一个偏工程实用的私有屏幕共享和 relay 工具，适合想自己部署、自己掌控链路、同时又希望观看体验尽量接近“点开就能看”的场景。
...
- **比单点推流更省房主上行**：多人观看时优先链式 relay，让观看端帮忙接力。
- **比黑盒服务更可控**：可以自部署信令服务器、更新源和后台，拓扑和诊断都能看见。
- **比临时方案更适合测试**：支持 native、OBS、Web、手机浏览器、局域网 HTTP 和 3010 实时拓扑后台。
